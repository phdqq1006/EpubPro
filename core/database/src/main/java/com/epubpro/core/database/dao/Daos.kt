...
interface BookDao {
    ...
    @Query("""
        ...
        INNER JOIN reading_progress ON reading_progress.bookId = books.id
        WHERE books.lastReadAt > 0 AND reading_progress.progressPercentage < 1.0
        ORDER BY books.lastReadAt DESC
        LIMIT 1
    """)
