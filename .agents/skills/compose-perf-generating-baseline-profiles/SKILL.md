---
name: compose-perf-generating-baseline-profiles
description: Tạo baseline profiles và macrobenchmark cho startup/frame timing. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: generating-baseline-profiles
upstream-path: measurement/generating-baseline-profiles
language: vi
---

# compose-perf-generating-baseline-profiles

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `generating-baseline-profiles`.
...
Dùng skill này khi task liên quan đến: **Tạo baseline profiles và macrobenchmark cho startup/frame timing.**
