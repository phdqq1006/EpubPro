---
name: compose-perf-ordering-modifier-chains
description: Review thứ tự modifier chain, then, padding/background/clip/clickable. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: ordering-modifier-chains
upstream-path: modifiers/ordering-modifier-chains
language: vi
---

# compose-perf-ordering-modifier-chains

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `ordering-modifier-chains`.
...
Dùng skill này khi task liên quan đến: **Review thứ tự modifier chain, then, padding/background/clip/clickable.**
