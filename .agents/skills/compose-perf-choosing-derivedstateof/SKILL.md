---
name: compose-perf-choosing-derivedstateof
description: Dùng derivedStateOf/snapshotFlow đúng trường hợp. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: choosing-derivedstateof
upstream-path: recomposition/choosing-derivedstateof
language: vi
---

# compose-perf-choosing-derivedstateof

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `choosing-derivedstateof`.
...
Dùng skill này khi task liên quan đến: **Dùng derivedStateOf/snapshotFlow đúng trường hợp.**
