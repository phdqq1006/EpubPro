---
name: cb-compose-state-and-effects
description: Quyết định ownership state, Flow collection, effect lifecycle, callback, snackbar, focus, navigation. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: chrisbanes/skills
upstream-skill: compose-state-and-effects
upstream-path: skills/compose-state-and-effects
language: vi
---

# cb-compose-state-and-effects

Nguồn gốc: chuyển thể standalone từ `chrisbanes/skills` / `compose-state-and-effects`.
...
Dùng skill này khi task liên quan đến: **Quyết định ownership state, Flow collection, effect lifecycle, callback, snackbar, focus, navigation.**
