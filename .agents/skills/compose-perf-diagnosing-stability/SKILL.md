---
name: compose-perf-diagnosing-stability
description: Đọc Compose Compiler reports để tìm unstable parameters/classes. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: diagnosing-compose-stability
upstream-path: stability/diagnosing-compose-stability
language: vi
---

# compose-perf-diagnosing-stability

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `diagnosing-compose-stability`.
...
Dùng skill này khi task liên quan đến: **Đọc Compose Compiler reports để tìm unstable parameters/classes.**
