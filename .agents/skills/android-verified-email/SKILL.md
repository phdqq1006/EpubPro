...
description: Tích hợp verified email/digital credential/passkey/Credential Manager đúng boundary bảo mật. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: android/skills
upstream-skill: verified-email
upstream-path: identity/verified-email
language: vi
---

# android-verified-email

Nguồn gốc: chuyển thể standalone từ `android/skills` / `verified-email`.
...
Dùng skill này khi task liên quan đến: **Tích hợp verified email/digital credential/passkey/Credential Manager đúng boundary bảo mật.**
