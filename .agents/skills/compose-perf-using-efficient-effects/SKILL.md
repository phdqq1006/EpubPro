---
name: compose-perf-using-efficient-effects
description: Chọn LaunchedEffect/DisposableEffect/RememberedEffect/rememberUpdatedState đúng key/lifecycle. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: using-efficient-effects
upstream-path: side-effects/using-efficient-effects
language: vi
---

# compose-perf-using-efficient-effects

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `using-efficient-effects`.
...
Dùng skill này khi task liên quan đến: **Chọn LaunchedEffect/DisposableEffect/RememberedEffect/rememberUpdatedState đúng key/lifecycle.**
