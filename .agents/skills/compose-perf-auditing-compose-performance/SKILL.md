---
name: compose-perf-auditing-compose-performance
description: Orchestrator chạy Measure → Diagnose → Fix → Verify cho Compose performance. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: auditing-compose-performance
upstream-path: audit/auditing-compose-performance
language: vi
---

# compose-perf-auditing-compose-performance

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `auditing-compose-performance`.
...
Dùng skill này khi task liên quan đến: **Orchestrator chạy Measure → Diagnose → Fix → Verify cho Compose performance.**
