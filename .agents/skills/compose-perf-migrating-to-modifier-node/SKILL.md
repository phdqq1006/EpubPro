---
name: compose-perf-migrating-to-modifier-node
description: Migrate custom modifier từ composed sang Modifier.Node. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: migrating-to-modifier-node
upstream-path: modifiers/migrating-to-modifier-node
language: vi
---

# compose-perf-migrating-to-modifier-node

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `migrating-to-modifier-node`.
...
Dùng skill này khi task liên quan đến: **Migrate custom modifier từ composed sang Modifier.Node.**
