---
name: compose-perf-setting-up-hotswan
description: Cài Compose HotSwan để có hot reload trên Android. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: setting-up-compose-hotswan
upstream-path: hot-reload/setting-up-compose-hotswan
language: vi
---

# compose-perf-setting-up-hotswan

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `setting-up-compose-hotswan`.
...
Dùng skill này khi task liên quan đến: **Cài Compose HotSwan để có hot reload trên Android.**
