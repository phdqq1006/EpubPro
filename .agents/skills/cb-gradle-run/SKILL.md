---
name: cb-gradle-run
description: Chạy Gradle qua wrapper output gọn và tách diagnostic owner khi cần. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: chrisbanes/skills
upstream-skill: gradle-run
upstream-path: skills/gradle-run
language: vi
---

# cb-gradle-run

Nguồn gốc: chuyển thể standalone từ `chrisbanes/skills` / `gradle-run`.
...
Dùng skill này khi task liên quan đến: **Chạy Gradle qua wrapper output gọn và tách diagnostic owner khi cần.**
