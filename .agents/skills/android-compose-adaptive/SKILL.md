...
description: Thiết kế UI Compose adaptive cho nhiều kích thước màn hình/foldable/window size. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: android/skills
upstream-skill: adaptive
upstream-path: jetpack-compose/adaptive
language: vi
---

# android-compose-adaptive

Nguồn gốc: chuyển thể standalone từ `android/skills` / `adaptive`.
...
Dùng skill này khi task liên quan đến: **Thiết kế UI Compose adaptive cho nhiều kích thước màn hình/foldable/window size.**
