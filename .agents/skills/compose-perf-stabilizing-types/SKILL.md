---
name: compose-perf-stabilizing-types
description: Ổn định type bằng immutable model, collections, annotation hoặc stability config. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: stabilizing-compose-types
upstream-path: stability/stabilizing-compose-types
language: vi
---

# compose-perf-stabilizing-types

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `stabilizing-compose-types`.
...
Dùng skill này khi task liên quan đến: **Ổn định type bằng immutable model, collections, annotation hoặc stability config.**
