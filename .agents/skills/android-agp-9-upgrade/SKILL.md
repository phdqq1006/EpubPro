...
description: Nâng cấp Android Gradle Plugin/Gradle/Kotlin và xử lý build system. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: android/skills
upstream-skill: agp-9-upgrade
upstream-path: build-system/agp/agp-9-upgrade
language: vi
---

# android-agp-9-upgrade

Nguồn gốc: chuyển thể standalone từ `android/skills` / `agp-9-upgrade`.
...
Dùng skill này khi task liên quan đến: **Nâng cấp Android Gradle Plugin/Gradle/Kotlin và xử lý build system.**
