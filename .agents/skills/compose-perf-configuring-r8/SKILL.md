---
name: compose-perf-configuring-r8
description: Cấu hình R8 full mode/consumer rules cho Compose performance. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: configuring-r8-for-compose
upstream-path: build/configuring-r8-for-compose
language: vi
---

# compose-perf-configuring-r8

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `configuring-r8-for-compose`.
...
Dùng skill này khi task liên quan đến: **Cấu hình R8 full mode/consumer rules cho Compose performance.**
