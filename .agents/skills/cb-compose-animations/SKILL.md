---
name: cb-compose-animations
description: Chọn Compose animation API đúng cho visibility, target value, transition, content swap. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: chrisbanes/skills
upstream-skill: compose-animations
upstream-path: skills/compose-animations
language: vi
---

# cb-compose-animations

Nguồn gốc: chuyển thể standalone từ `chrisbanes/skills` / `compose-animations`.
...
Dùng skill này khi task liên quan đến: **Chọn Compose animation API đúng cho visibility, target value, transition, content swap.**
