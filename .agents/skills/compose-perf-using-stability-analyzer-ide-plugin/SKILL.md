---
name: compose-perf-using-stability-analyzer-ide-plugin
description: Dùng Compose Stability Analyzer IDE plugin để triage instability. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: using-stability-analyzer-ide-plugin
upstream-path: stability/using-stability-analyzer-ide-plugin
language: vi
---

# compose-perf-using-stability-analyzer-ide-plugin

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `using-stability-analyzer-ide-plugin`.
...
Dùng skill này khi task liên quan đến: **Dùng Compose Stability Analyzer IDE plugin để triage instability.**
