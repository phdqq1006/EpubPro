---
name: compose-perf-debugging-recompositions
description: Debug recomposition count/skip count bằng Layout Inspector/instrumentation. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: debugging-recompositions
upstream-path: recomposition/debugging-recompositions
language: vi
---

# compose-perf-debugging-recompositions

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `debugging-recompositions`.
...
Dùng skill này khi task liên quan đến: **Debug recomposition count/skip count bằng Layout Inspector/instrumentation.**
