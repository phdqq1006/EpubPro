---
name: compose-perf-preserving-state-reloads
description: Giữ state qua reload bằng rememberSaveable/LazyListState/saveability. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: preserving-state-across-reloads
upstream-path: hot-reload/preserving-state-across-reloads
language: vi
---

# compose-perf-preserving-state-reloads

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `preserving-state-across-reloads`.
...
Dùng skill này khi task liên quan đến: **Giữ state qua reload bằng rememberSaveable/LazyListState/saveability.**
