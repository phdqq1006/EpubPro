---
name: compose-perf-using-strong-skipping-correctly
description: Dùng strong skipping, DontMemoize, NonSkippableComposable đúng tác dụng. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: using-strong-skipping-correctly
upstream-path: recomposition/using-strong-skipping-correctly
language: vi
---

# compose-perf-using-strong-skipping-correctly

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `using-strong-skipping-correctly`.
...
Dùng skill này khi task liên quan đến: **Dùng strong skipping, DontMemoize, NonSkippableComposable đúng tác dụng.**
