---
name: compose-perf-ai-mcp-hot-reload-loop
description: Vòng lặp AI edit → HotSwan reload → screenshot → inspect qua MCP. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: iterating-with-ai-and-mcp
upstream-path: hot-reload/iterating-with-ai-and-mcp
language: vi
---

# compose-perf-ai-mcp-hot-reload-loop

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `iterating-with-ai-and-mcp`.
...
Dùng skill này khi task liên quan đến: **Vòng lặp AI edit → HotSwan reload → screenshot → inspect qua MCP.**
