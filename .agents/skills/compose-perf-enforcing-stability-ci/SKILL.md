---
name: compose-perf-enforcing-stability-ci
description: Tạo CI gate cho stability report để không regression. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: enforcing-stability-in-ci
upstream-path: stability/enforcing-stability-in-ci
language: vi
---

# compose-perf-enforcing-stability-ci

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `enforcing-stability-in-ci`.
...
Dùng skill này khi task liên quan đến: **Tạo CI gate cho stability report để không regression.**
