---
name: compose-perf-deferring-state-reads
description: Dời state read từ Composition sang Layout/Draw bằng lambda modifiers/graphicsLayer/draw. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: deferring-state-reads
upstream-path: recomposition/deferring-state-reads
language: vi
---

# compose-perf-deferring-state-reads

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `deferring-state-reads`.
...
Dùng skill này khi task liên quan đến: **Dời state read từ Composition sang Layout/Draw bằng lambda modifiers/graphicsLayer/draw.**
