---
name: compose-perf-avoiding-subcomposition-pitfalls
description: Tránh lạm dụng SubcomposeLayout/BoxWithConstraints/Scaffold trong hot path. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: avoiding-subcomposition-pitfalls
upstream-path: recomposition/avoiding-subcomposition-pitfalls
language: vi
---

# compose-perf-avoiding-subcomposition-pitfalls

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `avoiding-subcomposition-pitfalls`.
...
Dùng skill này khi task liên quan đến: **Tránh lạm dụng SubcomposeLayout/BoxWithConstraints/Scaffold trong hot path.**
