---
name: compose-perf-collecting-flows-safely
description: Collect Flow bằng lifecycle-aware API, tránh drain battery/background work. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: collecting-flows-safely
upstream-path: side-effects/collecting-flows-safely
language: vi
---

# compose-perf-collecting-flows-safely

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `collecting-flows-safely`.
...
Dùng skill này khi task liên quan đến: **Collect Flow bằng lifecycle-aware API, tránh drain battery/background work.**
