---
name: compose-perf-configuring-lazy-prefetch
description: Cấu hình LazyLayoutCacheWindow/NestedPrefetch/pausable prefetch. Luôn xuất kết quả bằng tiếng Việt.
argument-hint: "<mục tiêu, diff, file hoặc lỗi cần xử lý>"
user-invocable: true
source-repo: skydoves/compose-performance-skills
upstream-skill: configuring-lazy-prefetch
upstream-path: lists/configuring-lazy-prefetch
language: vi
---

# compose-perf-configuring-lazy-prefetch

Nguồn gốc: chuyển thể standalone từ `skydoves/compose-performance-skills` / `configuring-lazy-prefetch`.
...
Dùng skill này khi task liên quan đến: **Cấu hình LazyLayoutCacheWindow/NestedPrefetch/pausable prefetch.**
